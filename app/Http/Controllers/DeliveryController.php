  ...
  public function AddAccountDeliveryManger(Request $request)
{
    ...
    // Check if there is already a delivery manager
    $deliveryManagerCount = Role::where('name', 'Delivery_manger')
        ->with('users')
        ->first()
        ->users
        ->count();

    if ($deliveryManagerCount > 0) {
        return response()->json(['error' => 'You ara added delivery manager already '], 400);
    }
...
public function AddAccountDelivery(Request $request)
{
    ...
    $currentWeek = date('W');
    $user->status = ($currentWeek % 2 == 0) ? (($user->id % 2 == 0) ? 'Active' : 'Inactive') : (($user->id % 2 == 0) ? 'Inactive' : 'Active');
    $user->save();
...
public function getActiveDeliveryWorkers()
{
    ...
    if ($role) {
        $activeDeliveryWorkers = $role->users()->where('status', 'Active')->get();
...
public function assignOrderToDelivery($orderId, $deliveryId) {
    ...
    $fcmToken=User::where('id',$deliveryId)->value('fcm_token');
    $message = [
     'title' =>'A new request has been assigned',
      'body' =>$orderId,
        ];
